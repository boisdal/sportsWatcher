...
angular.module('myApp.schedule', ['ngRoute'])
    ...
    .controller('ScheduleCtrl', function($scope, $http) {
        ...
        $http.get('https://www.thesportsdb.com/api/v1/json/1/eventsnextleague.php?id=4430').then(
            function successCallback(response) {
                $scope.response = response;
                $scope.upcoming15 = response.data.events;
                for (let match of $scope.upcoming15) {
                    $http.get('https://thesportsdb.com/api/v1/json/1/lookupteam.php?id='+match.idHomeTeam).then(
                        function successCallBack(response) {
                            $scope.response = response;
                    ...
                    $http.get('https://thesportsdb.com/api/v1/json/1/lookupteam.php?id='+match.idAwayTeam).then(
                        function successCallBack(response) {
                            $scope.response = response;
